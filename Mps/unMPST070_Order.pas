...
uses
   Classes, SysUtils, Variants, ComCtrls, Forms, DB, DBClient, StrUtils,
   unGlobal, unCommon, unMPST070_cdsxml;
...
begin
  ...
  tmpSQL:='Select Distinct Orderno,Orderitem,'
        +' Case When Srcflag%2=0 Then 2 Else 1 End Srcflag From MPS070'
        +' Where Len(IsNull(Orderno,''''))>0 And IsNull(ErrorFlag,0)=0'
  ...
  begin
    ...
    begin
      ...
      if FTmpCDS.Locate('Orderno;Orderitem;Srcflag',
            VarArrayOf([FieldByName('Orderno').AsString,
                        FieldByName('Orderitem').AsInteger,
...
begin
  ...
  begin
    ...
    while not Eof do
    begin
      if SameText(FieldByName('custno').AsString,'N006') and
         (FieldByName('srcflag').AsInteger mod 2<>0) then   
         tmpSQL:=tmpSQL+' or (oao01='+Quotedstr(FieldByName('orderno').AsString)
                       +' and oao03='+IntToStr(FieldByName('orderitem').AsInteger)+')';
      Next;
    end;
  end;
...
begin
  Result:='Select wono,orderno,orderdate,orderitem,materialno,sqty,'
         ...
         +' sdate1,machine1,Adate_new adate,'//cast(null as varchar(100))oao06,'
         +' case when (srcflag%2)=0 then 4 else 3 end as srcflag,'
         +' simuver+''@''+Cast(citem as varchar(10)) as errorid,orderno2,orderitem2'
         +' From MPS070 Where Bu='+Quotedstr(g_UInfo^.BU)
         +' And ErrorFlag=1 And IsNull(srcflag,0)>0 And Len(IsNull(Orderno,''''))>0';
end;
...
begin
  ...
  s   :='Select * From'
         ...
         +' B.ta_oeb02 pnlsize2,'+xSrcFlag+' srcflag,B.oeb05 unit,oao06,K.oea04,Q.occ02'
         ...
         +' left join iteqjx.oeb_file J on oao06 like ''JX-''||J.oeb01||''-''||J.oeb03||''%'''
         +' left join iteqjx.oea_file K on K.oea01=J.oeb01 '
         +' left join iteqjx.occ_file Q on K.oea04=Q.occ01 '
...
begin
  Result:='Select orderno,orderdate,orderitem,materialno,sqty,'
         +' orderQty,edate,custno,custom,custom2,materialno1,pnlsize1,pnlsize2,'
         +' sdate1,machine1,Adate_new adate,'
         +' case when (srcflag%2)=0 then 6 else 5 end as srcflag'
         +' From MPS070 Where Bu='+Quotedstr(g_UInfo^.BU)+xFilter
         +' And IsNull(ErrorFlag,0)=0 And IsNull(srcflag,0)>0 And Len(IsNull(Orderno,''''))>0';
end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                if len=12 then
                   FieldByName('materialno').AsString:=s1+Copy(tmpStr,2,9)+
                            FCDS3.FieldByName('tc_ocl07').AsString+Copy(tmpStr,11,2)
                else
                   FieldByName('materialno').AsString:=s1+Copy(tmpStr,2,9)+
                            FCDS3.FieldByName('tc_ocl07').AsString+Copy(tmpStr,19,2);
              end else
              begin
                if len=12 then
              ...
              if FCDS4.Locate('tc_ocm01',FieldByName('custom').AsString,[]) and
                 (FCDS4.FieldByName('tc_ocm03').AsInteger>0) then
              begin
                FieldByName('pnlnum').AsInteger:=FCDS4.FieldByName('tc_ocm03').AsInteger; //1開几數字
                FieldByName('sqty').AsFloat:=((FieldByName('sqty').AsFloat/FieldByName('pnlnum').AsInteger)*FieldByName('pnlsize1').AsFloat*25.4)/1000;
