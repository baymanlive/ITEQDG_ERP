...
const l_Xml='<?xml version="1.0" standalone="yes"?>'
           ...
           +'<FIELD attrname="pmk01" fieldtype="string" WIDTH="10"/>'
           +'<FIELD attrname="pml02" fieldtype="i4" WIDTH="5"/>'
...
var
  len, i: Integer;
  tmpSQL, tmpSQL2, tmpStr, tmpOraDB, tmpFstCode, tmpOrderno, tmpWono: string;
  Data: OleVariant;
...
begin
  ...
  try
    ...
    // tmpSQL := 'select C.*,oao06 from' + ' (select B.*,ima021 from' + ' (select A.*,occ02 from' + ' (select oea01,oea02,oea04,oeb03,oeb04,oeb05,oeb06,oeb11,oeb12,oeb15,' + ' oeb24,oeb12-oeb24 notqty,ta_oeb01,ta_oeb02,ta_oeb03,ta_oeb04,ta_oeb07,ta_oeb10,' + ' to_char(oea02,''YYYY/MM/DD'') q_oea02' + ' from oea_file inner join oeb_file on oea01=oeb01' + ' where oeaconf=''Y'' and nvl(oeb70,''N'')=''N'') A' + ' inner join occ_file on oea04=occ01 where 1=1 ' + xFliter + ' ) B left join ima_file on oeb04=ima01' + ' ) C left join oao_file on oea01=oao01 and oeb03=oao03' + ' order by oea02,oea01,oeb03';
    tmpSQL := 'select C.*,oao06,pmk01,pml02 from' +
