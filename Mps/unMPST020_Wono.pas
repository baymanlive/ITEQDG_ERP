...
constructor TMPST020_Wono.Create;
var
...
destructor TMPST020_Wono.Destroy;
begin
...
procedure TMPST020_Wono.ShowSB(msg:string);
begin
...
//取單頭
function TMPST020_Wono.GetWono_Fst(Machine, Custno, Premark: string; Fst: string = ''): string;
begin
...
//取線別
function TMPST020_Wono.GetLine(Orderno, Machine: string): string;
...
//取成本中心
function TMPST020_Wono.GetDeptno(Machine: string): string;
begin
...
//替換虛擬料號
function TMPST020_Wono.GetBomStr(IsDG:Boolean; BomStr,Pno,Custno,Adhesive:string):string;
var
...
//判斷sma_file參數及初始化
function TMPST020_Wono.Init(IsDG:Boolean): Boolean;
begin
  Result:=False;
  ShowSB('正在初始化...');
...
//刪除sfb
procedure TMPST020_Wono.Reset_sfb(OrcDB:string);
begin
...
//刪除ta_sfb
procedure TMPST020_Wono.Reset_ta_sfb(OrcDB:string);
begin
...
//刪除ecm
procedure TMPST020_Wono.Reset_ecm(OrcDB:string);
begin
...
//提交
function TMPST020_Wono.Post(IsDG:Boolean): Boolean;
var
  i:Integer;
...
//異常刪除
procedure TMPST020_Wono.DeleteRec(wono:string);
begin
...
//產生工單號碼
function TMPST020_Wono.SetWono(OrdWono:TOrderWono; var OutWono:string;Fst:string=''):Boolean;
var
...
begin
  ...
  begin
    ...
    FieldByName('ta_sfb02').AsString := GetLine(FieldByName('sfb22').AsString, OrdWono.Machine); //生產線別
 ...
 //FieldByName('ta_sfb09').AsString  := ''; //2up尺寸 tc_ksd46?
  ...
  begin
    ...
    begin
      ...
      //GetBomStr返回替換料號或本身料號
      tmpSQL:=GetBomStr(OrdWono.IsDG, FtmpCDS.FieldByName('bmb03').AsString,
                        OrdWono.Pno, OrdWono.Custno, OrdWono.Adhesive);
      ...
      if FtmpCDS.FieldByName('ima64').AsFloat<>0 then
         tmpQty:=(tmpQty/FtmpCDS.FieldByName('ima64').AsFloat+0.999999)*
                 FtmpCDS.FieldByName('ima64').AsFloat; }
                 
      if SameText(FieldByName('sfa12').AsString,'KG') then
         FieldByName('sfa05').AsFloat := RoundTo(FieldByName('sfa04').AsFloat,-2)
      else
         FieldByName('sfa05').AsFloat := Trunc(FieldByName('sfa04').AsFloat);
