...
procedure TFrmMPSI260.SetToolBar;
begin
  btn_mpsi260.Enabled:=g_MInfo^.R_edit and CDS.Active and (not (CDS.State in [dsInsert,dsEdit]));
  inherited;
end;

procedure TFrmMPSI260.RefreshDS(strFilter:string);
var
  tmpSQL:string;
  Data:OleVariant;
begin
  tmpSQL:='Select * From MPS260 Where Bu='+Quotedstr(g_UInfo^.BU)+' '+strFilter
         +' And IsNull(IsDG,0)=1 Order By Custno,Adhesive,Fiber,Code3,LastCode3,TailNo,Vendor';
...
procedure TFrmMPSI260.FormCreate(Sender: TObject);
begin
  p_SysId:='Mps';
  p_TableName:='MPS260';
...
procedure TFrmMPSI260.CDSNewRecord(DataSet: TDataSet);
var
  i:Integer;
begin
  ...
  inherited;
  DataSet.FieldByName('IsDG').AsBoolean:=True;
end;

procedure TFrmMPSI260.CDSBeforePost(DataSet: TDataSet);
var
  tmpSQL:string;
  Data:OleVariant;
...
procedure TFrmMPSI260.btn_mpsi260Click(Sender: TObject);
var
...
procedure TFrmMPSI260.DBGridEh1GetCellParams(Sender: TObject;
