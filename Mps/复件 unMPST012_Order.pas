...
uses
   ...
   unGlobal, unCommon, unMPST012_units,ADODB;
...
begin
  ...
  tmpSQL:='Select upper(A.custno) custno1,upper(B.custno) as custno2,B.adhesive,'
         +' B.strip_lower,B.strip_upper,B.cwcode_lower,B.cwcode_upper,B.qty_lower,code9_11,code15,'
         +' B.qty_upper,B.Addqty From MPS190 as A full join MPS160 as B'
         +' on A.Bu=B.Bu Where A.Bu='+Quotedstr(g_UInfo^.BU);
  ...
  //料號與OZ關系
  Data1:=null;
  tmpSQL:='Select Materialno,OZ From MPS150'
...
begin
  ...
  tmpSQL:='Select Distinct Orderno,Orderitem,'
        +' Case When Srcflag%2=0 Then 2 Else 1 End Srcflag From MPS010'
        +' Where Bu='+Quotedstr(g_UInfo^.BU)+' And IsNull(ErrorFlag,0)=0'
        +' Union'
        +' Select Distinct Orderno,Orderitem,'
        +' Case When Srcflag%2=0 Then 2 Else 1 End Srcflag From MPS012'
        +' Where Bu='+Quotedstr(g_UInfo^.BU);
...
//  FTmpCDS.Data:=Data1;
  ...
  begin
    ...
    begin
      ...
      if {FTmpCDS}FrmMPST012.ADOQuery1.Locate('Orderno;Orderitem;Srcflag',
     ...
     s:='';
{
  ...
  Result := 'Select * From'
         +' (Select A.oea01 orderno,to_char(A.oea02,'''+g_cShortDate1+''') orderdate,'
       //  +' B.oeb03 orderitem,B.oeb04 materialno,B.oeb12 sqty,B.oeb12 orderQty,'
        +' B.oeb03 orderitem,B.oeb04 materialno,nvl(B.oeb12,0)-nvl(B.oeb905,0) sqty,nvl(B.oeb12,0)-nvl(B.oeb905,0)  orderQty,'
         +' to_char(B.oeb15,'''+g_cShortDate1+''') edate,A.oea04 custno,'
         +' B.ta_oeb04 materialno1,B.ta_oeb07 custom,B.ta_oeb01 pnlsize1,'
         +' B.ta_oeb02 pnlsize2,substr(B.oeb04,3,4) thickness,'
         +' substr(B.oeb04,7,2) oz,'+xSrcFlag+' srcflag,'
         +' concat(ta_oeb05,ta_oeb06) supplier,'
         +' ''' + xBu + ''' as orderBu,'
         +' cast('''' as varchar2(100)) as premark3,'
         +' cast(lower(RAWTOHEX(sys_guid())) as VARCHAR (50)) as uuid '
         +' From '+xBu+'.oea_file A Inner Join '+xBu+'.oeb_file B On A.oea01=B.oeb01'
         +' Where A.oeaconf<>''X'' and B.oeb12>0 '+FOrderType+s
         +' And Substr(B.oeb04,1,1) in (''E'',''T'')) C Where 1=1 '+xFilter;
end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              if len=11 then
                 FieldByName('materialno').AsString:=Copy(tmpStr,1,8)+
                        FCDS3.FieldByName('tc_ocl07').AsString+Copy(tmpStr,9,3)
              else
                 FieldByName('materialno').AsString:=Copy(tmpStr,1,8)+
                        FCDS3.FieldByName('tc_ocl07').AsString+Copy(tmpStr,17,3);
            end else
            begin
              if len=11 then
          ...
          //良率
//          if Pos(Copy(tmpStr,9,6),'740490,820490,860490')=0 then
          ...
          begin
            ...
            begin
              ...
              begin
                if (Pos(FieldByName('custno').AsString, FCDS5.FieldByName('custno2').AsString)>0) and
                   (Pos(Copy(tmpStr,2,1), FCDS5.FieldByName('adhesive').AsString)>0) then
                begin
                  isErr:=True;
