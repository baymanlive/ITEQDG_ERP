...
uses
  ...
  Dialogs, DB, ADODB, StdCtrls, ExtCtrls, XPMan, DBClient, Provider,
  ComCtrls, Mask, DBCtrls, DBCtrlsEh, Math;
...
uses unIPQCT620_login, unIPQCT620_waste_pno;
...
begin
  ...
  begin
    ...
    SQL.Text:='select a.username,b.r_garbage from sys_user a,sys_userright b'
             ...
             +' and a.userid='+Quotedstr(l_uid)
             +' and a.password='+Quotedstr(pw)
             +' and isnull(a.not_use,0)=0 and b.procid=''IPQCT620'''; //and b.r_visible=1 由主畫面啟動,r_visible=1
    try
      Open;
    except
      Exit;
...
begin
  ...
  begin
    ...
    if (CDS.FieldByName('in_kg').AsFloat<v1) or
       (CDS.FieldByName('in_kg').AsFloat>v2) then
    begin
      Application.MessageBox(PAnsiChar('實際投入量(kg),不在設定範圍內('+FloatToStr(v1)+'~'+FloatToStr(v2)+')!'),l_hint,48);
      if DBEdit1.CanFocus then
         DBEdit1.SetFocus;
      Exit;
    end;
...
begin
  ...
  if (Length(CDS.FieldByName('iuser').AsString)>0) and
     (not SameText(CDS.FieldByName('iuser').AsString,l_uid)) then
  begin
    cbb3.Enabled:=False;
...
procedure TFrmIPQCT620.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  ...
  ADOQuery1.SQL.Text:='select a.remark,b.* from ipqc610 a,ipqc611 b'
                     +' where a.bu=b.bu and a.ad=b.ad and a.ver=b.ver'
                     +' and a.bu='+Quotedstr(l_bu)
                     +' and a.conf=1 and a.ad='+Quotedstr(Edit1.Text)
                     +' and a.ver='+Quotedstr(cbb2.Items.Strings[cbb2.ItemIndex])
                     +' order by b.sno';
  try
    ADOQuery1.Open;
  except
  ...
  begin
    ...
    begin
      ...
      if not FieldByName('istext').AsBoolean then
         FieldByName('kg').AsFloat:=RoundTo(FieldByName('kg').AsFloat*l_qty-FieldByName('keep').AsFloat,-6);    //計劃量
...
var
  ...
  var
    tmpSQL:string;
  begin
    tmpSQL:=' delete from ipqc620 '+tmpFilter
           +' delete from ipqc621 '+tmpFilter;
    try
      Conn.Execute(tmpSQL);
    except
    end;
...
begin
  ...
  begin
    if Application.MessageBox(PAnsiChar('['+tmpAd+','+tmpVer+','+tmpLot+']'+#13#10+'此膠系+版本的批號資料已存在,確定覆蓋嗎?'),l_hint,33)=IdCancel then
       Exit;
  end;
...
begin
  ...
  ADOQuery4.SQL.Text:='select ver from ipqc610'
                     +' where bu='+Quotedstr(l_bu)
                     +' and conf=1 and ad='+Quotedstr(Edit1.Text)
                     +' order by ver';
  try
    ADOQuery4.Open;
  except
