...
function TFrmDLII040_prn.FindNumber(SourceStr:WideString;
...
var
  tmpMsg:string;
begin
  if Length(Trim(DS.DataSet.FieldByName('B1').AsString))=0 then
     tmpMsg:=tmpMsg+l_SpecificationSet;
  if Length(Trim(DS.DataSet.FieldByName('B11').AsString))=0 then
     tmpMsg:=tmpMsg+l_TestValueSet;

  if (Length(tmpMsg)=0) and
     (StrToFloatDef(Trim(DS.DataSet.FieldByName('B11').AsString), 0)<=0) then
  ...
  tmpMsg:='';
  if Length(Trim(DS.DataSet.FieldByName('B2').AsString))=0 then
     tmpMsg:=tmpMsg+l_SpecificationSet;
  if Length(Trim(DS.DataSet.FieldByName('B21').AsString))=0 then
     tmpMsg:=tmpMsg+l_TestValueSet;

  if (Length(tmpMsg)=0) and
     (StrToFloatDef(Trim(DS.DataSet.FieldByName('B21').AsString), 0)<=0) then
     tmpMsg:=tmpMsg+'測試值不是有效的正數值'+l_enter;
  ...
  begin
    ...
    if pos1=0 then
      ...
      if (num1<=0) or (num2<=0) or (num1>=num2) then
        ...
        begin
          if l_diff<=0 then
             tmpMsg:=tmpMsg+DS.DataSet.FieldByName('C1').AsString+
                           '公差值錯誤:'+FloatToStr(l_diff)+l_enter
          else begin
        ...
        begin
          if DS.DataSet.FieldByName('C1').AsString='+0/-6.3' then
          begin
            if (num1<=56.7) or (num2>=63) then
               tmpMsg:=tmpMsg+'測試值不在規格要求範圍內:56.7~63'+l_enter;
          end else
          begin
            pos1:=Pos('+', DS.DataSet.FieldByName('C1').AsString);
            pos2:=Pos('/-', DS.DataSet.FieldByName('C1').AsString);
            if (pos1>0) and (pos2>0) and (pos2-pos1>1) then //+a/-b
            begin
              str1:=StringReplace(StringReplace(DS.DataSet.FieldByName('C1').AsString,'+','',[rfReplaceAll]),'/-',',',[rfReplaceAll]);
              pos1:=Pos(',',str1);
            ...
            begin
              num3:=Abs(FindNumber(DS.DataSet.FieldByName('C1').AsString));
              if num3=0 then
                 tmpMsg:=tmpMsg+'規格要求[公差]不是有效的正數值'+l_enter
              else begin
  ...
  begin
    ...
    if pos1=0 then
      ...
      if (num1<=0) or (num2<=0) or (num1>=num2) then
        ...
        begin
          if l_diff<=0 then
             tmpMsg:=tmpMsg+DS.DataSet.FieldByName('D1').AsString+
                           '公差值錯誤:'+FloatToStr(l_diff)+l_enter
          else begin
        ...
        begin
          ...
          begin
            num3:=Abs(FindNumber(DS.DataSet.FieldByName('D1').AsString));
            if num3=0 then
               tmpMsg:=tmpMsg+'規格要求[公差]不是有效的正數值'+l_enter
            else begin
...
var
  ...
  num1,num2,num3:Double;
begin
  if (Length(Trim(DS.DataSet.FieldByName('E1').AsString))=0) or
     (Length(Trim(DS.DataSet.FieldByName('E2').AsString))=0) then
     tmpMsg:=tmpMsg+l_SpecificationSet;
  if (Length(Trim(DS.DataSet.FieldByName('E11').AsString))=0) or
     (Length(Trim(DS.DataSet.FieldByName('E21').AsString))=0) then
     tmpMsg:=tmpMsg+l_TestValueSet;
  ...
  begin
    ...
    if num1<=0 then
       tmpMsg:=tmpMsg+'1.測試值不是有效的正數值'+l_enter;
    ...
    if num1<=0 then
       tmpMsg:=tmpMsg+'2.測試值不是有效的正數值'+l_enter;
...
var
  ...
  num1,num2:Double;
begin
  if (Length(Trim(DS.DataSet.FieldByName('G1').AsString))=0) or
     (Length(Trim(DS.DataSet.FieldByName('G2').AsString))=0) then
     tmpMsg:=tmpMsg+l_SpecificationSet;
  if (Length(Trim(DS.DataSet.FieldByName('G11').AsString))=0) or
     (Length(Trim(DS.DataSet.FieldByName('G21').AsString))=0) then
     tmpMsg:=tmpMsg+l_TestValueSet;
  ...
  begin
    ...
    if num1<=0 then
      ...
      if num2<=0 then
         tmpMsg:=tmpMsg+'1.規格要求不是有效的正數值'+l_enter
    ...
    if num1<=0 then
      ...
      if num2<=0 then
         tmpMsg:=tmpMsg+'2.規格要求不是有效的正數值'+l_enter
      else if num2>num1 then
         tmpMsg:=tmpMsg+'2.測試值不在規格要求範圍內'+l_enter;
    end;
  end;
...
begin
  ...
  if (Length(tmpMsg)=0) and
     SameText(DS.DataSet.FieldByName('H12').AsString, 'pass') then
  begin
...
begin
  ...
  if (Length(tmpMsg)=0) and
     SameText(DS.DataSet.FieldByName('H22').AsString, 'pass') then
  begin
    num1:=StrToFloatDef(DS.DataSet.FieldByName('H21').AsString, 0);
    if num1>=0 then
       tmpMsg:=tmpMsg+'測試值不是有效的負數值'+l_enter;

    pos1:=Pos('±', DS.DataSet.FieldByName('H2').AsString);
    if pos1=0 then
       tmpMsg:=tmpMsg+'規格要求格式錯誤(#±123)'+l_enter
    else begin
      str1:=Copy(DS.DataSet.FieldByName('H2').AsString, 1, pos1-1);
      str2:=Copy(DS.DataSet.FieldByName('H2').AsString, pos1+2, 20);
    end;
...
var
  ...
  num1,num2,num3:Double;
begin
  if SameText(DS.DataSet.FieldByName('H12').AsString, 'pass') then
  begin
    num1:=StrToFloatDef(DS.DataSet.FieldByName('H11').AsString, 0);
    if num1>=0 then
       tmpMsg:=tmpMsg+'測試值不是有效的負數值'+l_enter
...
begin
  ...
  begin
    ...
    num4:=0;                                                  //tg2
                                                              //num5 △tg1
    if num1<=0 then
       tmpMsg:=tmpMsg+'Tg規格要求不是有效的正數值'+l_enter;
    if num2<=0 then
       tmpMsg:=tmpMsg+'△Tg規格要求不是有效的正數值'+l_enter;
    ...
    if pos1=0 then
      ...
      if num3<=0 then
         tmpMsg:=tmpMsg+'Tg1測試值不是有效的正數值'+l_enter
    ...
    if pos1=0 then
      ...
      if num4<=0 then
         tmpMsg:=tmpMsg+'Tg2測試值不是有效的正數值'+l_enter
    ...
    if num5<=0 then
       tmpMsg:=tmpMsg+'△Tg測試值不是有效的正數值'+l_enter
    else if SameText(l_SMRec.Custno, 'AC093') then
    begin
    ...
    if (num3>0) and (num4>0) and (num5>0) and
       (Abs(Abs(num4-num3)-num5)>0.000001) then
...
begin
  ...
  if num1<=0 then
     tmpMsg:=tmpMsg+'Tg未設定或不是有效的正數值';
  if num2<=0 then
     tmpMsg:=tmpMsg+'Tg1未設定或不是有效的正數值';
...
begin
  ...
  begin
    ...
    num2:=FindNumber(str1);
    if num1<=0 then
       tmpMsg:=tmpMsg+'規格要求不是有效的正數值'+l_enter;
    if num2<=0 then
       tmpMsg:=tmpMsg+'測試值不是有效的正數值'+l_enter;
    ...
    begin
      ...
      begin
        if pos1=0 then
          ...
          if num2<=0 then
             tmpMsg:=tmpMsg+'AC148/AC347 1g測試值不是有效的正數值'+l_enter
          else if num2>num1 then
...
begin
  ...
  if (Length(tmpMsg)=0) and
     (StrToFloatDef(Trim(DS.DataSet.FieldByName('M11').AsString), 0)<=0) then
     tmpMsg:=tmpMsg+'測試值不是有效的正數值'+l_enter;
...
var
  ...
  num1,num2:Double;
begin
  if (Length(Trim(DS.DataSet.FieldByName('O1').AsString))=0) or
     (Length(Trim(DS.DataSet.FieldByName('O2').AsString))=0) or
     (Length(Trim(DS.DataSet.FieldByName('O3').AsString))=0) then
     tmpMsg:=tmpMsg+l_SpecificationSet;
  if (Length(Trim(DS.DataSet.FieldByName('O11').AsString))=0) or
     (Length(Trim(DS.DataSet.FieldByName('O21').AsString))=0) or
     (Length(Trim(DS.DataSet.FieldByName('O31').AsString))=0) then
     tmpMsg:=tmpMsg+l_TestValueSet;
  ...
  begin
    ...
    begin
      ...
      if num1<=0 then
         tmpMsg:=tmpMsg+'3.規格要求不是有效的正數值'+l_enter;
      if num2<=0 then
         tmpMsg:=tmpMsg+'3.測試值不是有效的正數值'+l_enter;
      if (num1>0) and (num2>0) and (num2>num1) then
...
var
  ...
  num1,num2:Double;
begin
  if (Length(Trim(DS.DataSet.FieldByName('R1').AsString))=0) or
     (Length(Trim(DS.DataSet.FieldByName('R2').AsString))=0) then
     tmpMsg:=tmpMsg+l_SpecificationSet;
  if (Length(Trim(DS.DataSet.FieldByName('R11').AsString))=0) or
     (Length(Trim(DS.DataSet.FieldByName('R21').AsString))=0) then
     tmpMsg:=tmpMsg+l_TestValueSet;
...
var
  tmpMsg:string;
begin
  if (DS.DataSet.FieldByName('Resin_visible').AsString='1') and
     (Length(Trim(DS.DataSet.FieldByName('Resin').AsString))=0) then
     tmpMsg:=tmpMsg+'樹脂未輸入'+l_enter;

  if (DS.DataSet.FieldByName('PP_visible').AsString='1') and
     (Length(Trim(DS.DataSet.FieldByName('PP').AsString))=0) then
     tmpMsg:=tmpMsg+'玻布未輸入'+l_enter;

  if (DS.DataSet.FieldByName('Copper_visible').AsString='1') and
     (Length(Trim(DS.DataSet.FieldByName('Copper').AsString))=0) then
...
procedure TFrmDLII040_prn.FormClose(Sender: TObject;
...
begin
  ...
  begin
    ...
    if Pos(l_SMRec.Custno, 'AC394/AC152/AC844/AC405/AC075/AC310/AC311/AC950')>0 then
       Item_R;  //抗彎曲強度
