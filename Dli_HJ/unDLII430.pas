...
const l_CDSXml='<?xml version="1.0" standalone="yes"?>'
              ...
              +'<FIELD attrname="transport" fieldtype="string" WIDTH="20"/>'
              +'<FIELD attrname="pricetype" fieldtype="string" WIDTH="20"/>'
...
begin
  ...
  begin
    if (not CDS.Active) or CDS.IsEmpty then
        tmpSQL:='Select Sno,Custno,Custshort,Orderno,Orderitem,Pno,Notcount1,Units,Remark,Delcount1,Coccount1'
        ...
        tmpSQL:='Select Sno,Custno,Custshort,Orderno,Orderitem,Pno,Notcount1,Units,Remark,Delcount1,Coccount1'
               ...
               +' And Len(IsNull(Saleno,''''))=0 Order By Sno';
...
//  inherited;
  ...
  if CDS.FieldByName('state').AsString<>CheckLang('已出車') then
  begin
  ...
  try
    ...
    if Length(CDS.FieldByName('cno').AsString)>0 then
       ...
       l_CDS.FieldByName('id').AsString:=FormatDateTime('YYYYMMDD',CDS.FieldByName('Indate').AsDateTime)
                                        +RightStr(IntToStr((10000+CDS.FieldByName('id').AsInteger)),2);
    ...
    l_CDS.FieldByName('transport').AsString:=Trim(CDS.FieldByName('transport').AsString);
    l_CDS.FieldByName('pricetype').AsString:=Trim(CDS.FieldByName('pricetype').AsString);
    ...
    begin
      ...
      begin
        ...
        Delete(tmpSQL,1,1);
        Data:=null;
        tmpSQL:='Select A.*,occ02 From (Select oga01,oga04 From oga_file'
               +' Where oga01 in ('+tmpSQL+')) A Left Join occ_file'
