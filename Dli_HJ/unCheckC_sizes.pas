...
begin
  ...
  Data:=null;
  tmpSQL:='Select CodeId,upper(Custno) Custno,upper(Code) Code,'
         +' upper(LstCode) LstCode,StdValue,StdValueNo,StdValueOne'
    ...
    begin
      ...
      begin
        ...
        if (FieldByName('LstCode').AsString='0') or
           (Pos(','+UpperCase(LstCode)+',',','+FieldByName('LstCode').AsString+',')>0) then
           Inc(tmpNum)
        else
        begin
          Next;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if not IsEmpty then
        begin
          if not Locate('Custno;Adhesive', VarArrayOf([SMRec.Custno, SMRec.M2]), []) then
            if not Locate('Custno;Adhesive', VarArrayOf(['@', SMRec.M2]), []) then
               Locate('Custno;Adhesive', VarArrayOf(['@','@']), []);
          tmpSQL:='Struct'+SMRec.M15;
          if FindField(tmpSQL)<>nil then
...
begin
  ...
  if (Pos(SMRec.M9_11+'*'+SMRec.M12_14,C_Sizes)=0) and
     ...
     (Pos('經'+tmpSizes1+'mm*緯'+tmpSizes2+'mm',C_Sizes)=0) then
  begin
