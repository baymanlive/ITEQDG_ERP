...
type
  ...
  public
    ...
    function GetLot(One:Boolean):string;
    { Public declarations }
  end;
...
begin
  tmpSQL:='Select manfac,min(sno) sno,sum(qty) qty From Dli040'
         +' Where Dno='+Quotedstr(Dno)
         +' And Ditem='+Ditem
         +' And Bu='+Quotedstr(g_UInfo^.BU)
         +' And Isnull(qty,0)<>0'
         +' Group By manfac Order By sno,manfac';
    ...
    try
      ...
      begin
        TreeView1.Items.AddChild(nil, tmpCDS.Fields[0].AsString
                      +'('+FloatToStr(tmpCDS.Fields[2].AsFloat)+Units+')');
...
//返回SQL語句的And條件,One=True只要一個批號
function TFrmDLII040_lot.GetLot(One:Boolean):string;
var
  i:Integer;
  tmpStr:string;
begin
...
procedure TFrmDLII040_lot.btn_okClick(Sender: TObject);
var
  i,cnt:Integer;
begin
  cnt:=0;
