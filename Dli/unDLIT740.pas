...
var
  FrmDLIT740: TFrmDLIT740;

implementation

uses unGlobal, unCommon, unDLIT7XX_units, unDLIT7XX_select;

const l_FormatStr='D74-YYMMDD999';

{$R *.dfm}

procedure TFrmDLIT740.CheckConf;
var
  tmpSQL:string;
  Data:OleVariant;
begin
...
//flag=0/1
procedure TFrmDLIT740.Conf(flag:string);
var
  v:Integer;
  tmpSQL:string;
  Data:OleVariant;
...
begin
  ...
  tmpSQL:='exec [dbo].[proc_DLIT740] '+Quotedstr(g_UInfo^.BU)+','+Quotedstr(CDS.FieldByName('cno').AsString)+','+flag;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        ShowMsg(tmpCDS.Fields[1].AsString+'棧板未出貨,不可回收!',48)
      else if v=-7 then
        ShowMsg(tmpCDS.Fields[1].AsString+'棧板無此客戶出貨記錄,不可回收!',48)
      else
...
procedure TFrmDLIT740.CdateChange(Sender: TField);
begin
...
procedure TFrmDLIT740.LotChange(Sender: TField);
var
  tmpSQL:string;
  tmpCDS:TClientDataSet;
  Data:OleVariant;
...
procedure TFrmDLIT740.RefreshDS2;
var
  tmpSQL:string;
begin
...
procedure TFrmDLIT740.FormCreate(Sender: TObject);
begin
  p_SysId:='Dli';
  p_MainTableName:='DLI740';
  p_DetailTableName:='DLI741';
...
procedure TFrmDLIT740.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrmDLIT740.btn_editClick(Sender: TObject);
begin
  CheckConf;
  inherited;
end;

procedure TFrmDLIT740.btn_deleteClick(Sender: TObject);
begin
  CheckConf;
  inherited;
end;

procedure TFrmDLIT740.btn_cancelClick(Sender: TObject);
begin
  inherited;
  imgConf.Visible:=CDS.FieldByName('Conf').AsBoolean;
end;

procedure TFrmDLIT740.btn_queryClick(Sender: TObject);
var
  tmpStr:string;
begin
  //inherited;
...
procedure TFrmDLIT740.btn_confClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmDLIT740.CDSNewRecord(DataSet: TDataSet);
begin
  inherited;
...
procedure TFrmDLIT740.CDSAfterOpen(DataSet: TDataSet);
begin
  inherited;
  DataSet.FieldByName('cdate').OnChange:=CdateChange;
end;

procedure TFrmDLIT740.CDS2AfterOpen(DataSet: TDataSet);
begin
  inherited;
  DataSet.FieldByName('lot').OnChange:=LotChange;
end;

procedure TFrmDLIT740.CDSBeforePost(DataSet: TDataSet);
var
  tmpSQL:string;
  isExist:Boolean;
begin
...
procedure TFrmDLIT740.CDS2BeforePost(DataSet: TDataSet);
begin
  ...
  begin
    ...
    if DBGridEh2.CanFocus then
       DBGridEh2.SetFocus;
    DBGridEh2.SelectedField:=DataSet.FieldByName('custno');
    Abort;
  end;
...
procedure TFrmDLIT740.CDSBeforeInsert(DataSet: TDataSet);
begin
  l_cno:='';
  inherited;
end;

procedure TFrmDLIT740.CDSBeforeDelete(DataSet: TDataSet);
begin
  inherited;
  l_cno:=DataSet.FieldByName('cno').AsString;
end;

procedure TFrmDLIT740.CDSBeforeEdit(DataSet: TDataSet);
begin
  inherited;
  l_cno:=DataSet.FieldByName('cno').AsString;
end;

procedure TFrmDLIT740.CDSAfterDelete(DataSet: TDataSet);
var
  tmpSQL:string;
begin
  inherited;
...
procedure TFrmDLIT740.CDSAfterPost(DataSet: TDataSet);
var
  tmpSQL:string;
begin
  inherited;
...
procedure TFrmDLIT740.CDSAfterScroll(DataSet: TDataSet);
begin
  inherited;
  imgConf.Visible:=DataSet.FieldByName('Conf').AsBoolean;
end;

procedure TFrmDLIT740.CDS2NewRecord(DataSet: TDataSet);
begin
  inherited;
  DataSet.FieldByName('cno').AsString:=CDS.FieldByName('cno').AsString;
end;

procedure TFrmDLIT740.CDS2BeforeInsert(DataSet: TDataSet);
begin
  CheckConf;
  inherited;
end;

procedure TFrmDLIT740.CDS2BeforeEdit(DataSet: TDataSet);
begin
  CheckConf;
  inherited;
end;

procedure TFrmDLIT740.CDS2BeforeDelete(DataSet: TDataSet);
begin
  CheckConf;
  inherited;
end;

procedure TFrmDLIT740.DBGridEh2EditButtonClick(Sender: TObject);
begin
  inherited;
  ...
  try
    FrmDLIT7XX_select.l_SQLFilter:=' and [state] like '+Quotedstr('出貨%');
    ...
    begin
      ...
      CDS2.FieldByName('custno').AsString:=StringReplace(FrmDLIT7XX_select.CDS.FieldByName('state').AsString,'出貨:','',[]);
    end;
  finally
...
procedure TFrmDLIT740.Timer1Timer(Sender: TObject);
var
  tmpSQL:string;
  Data:OleVariant;
begin
