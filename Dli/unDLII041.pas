...
uses
  ...
  StdCtrls, ToolWin, unDLII041_rpt;

type
  TFrmDLII041 = class(TFrmSTDI041)
    PCL2: TPageControl;
    TabSheet2: TTabSheet;
    DBGridEh2: TDBGridEh;
    CDS2: TClientDataSet;
...
procedure TFrmDLII041.CDS2_Edit_Delete(isDelete: Boolean);
var
...
procedure TFrmDLII041.RefreshDS(strFilter: string);
var
  tmpSQL: string;
  Data: OleVariant;
begin
  tmpSQL := 'Select * From ' + p_TableName + ' Where Bu=' + Quotedstr(g_UInfo^.Bu) + ' ' + strFilter +
    ' And Substring(Pno,1,1) not in (''E'',''T'')' + ' And Custno is not null And Custno<>''''' +
...
procedure TFrmDLII041.RefreshDS2;
var
  tmpSQL: string;
begin
...
procedure TFrmDLII041.RefreshDS3;
var
  tmpSQL: string;
begin
  tmpSQL := 'Select * From ' + l_tb3 + ' Where Dno=' + Quotedstr(CDS.FieldByName('Dno').AsString)
    + ' And Ditem=' + IntToStr(CDS.FieldByName('Ditem').AsInteger) + ' And Bu=' + Quotedstr(g_UInfo^.BU) +
    ' Order By Sno';
  l_list3.Insert(0, tmpSQL);
end;

procedure TFrmDLII041.FormCreate(Sender: TObject);
begin
  p_SysId := 'Dli';
  p_TableName := 'Dli010';
...
procedure TFrmDLII041.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrmDLII041.CDSAfterScroll(DataSet: TDataSet);
begin
  inherited;
  RefreshDS2;
  RefreshDS3;
...
procedure TFrmDLII041.btn_deleteClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmDLII041.btn_printClick(Sender: TObject);
var
  tmpCDS: TClientdataset;
  remark, custno, custpno, custpname, custpo, custname: string;
  i: integer;
  custInfo:TCustInfo;
...
//  if (Pos(custno, 'AC121/AC820/ACA97/AC109/AC172/ACE06/AC145/ACE67/ACD57') = 0) then
//  begin
//    l_DLII041_rpt.StartPrint(g_MInfo^.ProcId, CDS3.fieldbyname('qrcode').asstring);
//    Exit;
//  end;
  custno:=custInfo.No;
   custname:=custInfo.Name; custpno:=custInfo.PartNo;
    custpname:=custInfo.PartName;
    custpo:=custInfo.Po;
  {(*}
  if (Pos(custno, 'AC365/AC388/AC434/ACD39/ACF29') > 0) and
     (Pos('能源板',CDS.FieldByName('Custprono').AsString) >0) and
  ...
  if SameText('AC360',custno) and
     (Copy(CDS.FieldByName('custno').AsString,2,3) = '207') and
     (Pos(Copy(CDS.FieldByName('pno').AsString,3,1),'68D')=0)    {*)}
  then
  begin
  ...
  try
    ...
    l_DLII041_rpt.FSourceCDS := CDS;
    tmpCDS.free;
  end;
//  l_DLII041_rpt.StartPrint(g_MInfo^.ProcId, CDS3.fieldbyname('qrcode').asstring);
end;

procedure TFrmDLII041.btn_queryClick(Sender: TObject);
var
  tmpStr: string;
begin
//  inherited;
...
procedure TFrmDLII041.DBGridEh1CellClick(Column: TColumnEh);
begin
  inherited;
...
procedure TFrmDLII041.CDSAfterPost(DataSet: TDataSet);
var
  tmpSQL: string;
begin
  inherited;
...
procedure TFrmDLII041.CDS2AfterPost(DataSet: TDataSet);
begin
  inherited;
...
procedure TFrmDLII041.CDS3AfterPost(DataSet: TDataSet);
var
  tmpSQL: string;
begin
  inherited;
...
procedure TFrmDLII041.CDSBeforeInsert(DataSet: TDataSet);
begin
  Abort;
//  inherited;
end;

procedure TFrmDLII041.CDS2BeforeInsert(DataSet: TDataSet);
begin
  Abort;
//  inherited;
end;

procedure TFrmDLII041.CDS3BeforeInsert(DataSet: TDataSet);
begin
  Abort;
//  inherited;
end;

procedure TFrmDLII041.CDS2BeforeEdit(DataSet: TDataSet);
begin
...
procedure TFrmDLII041.CDS3BeforeEdit(DataSet: TDataSet);
begin
  if not g_MInfo^.R_delete then
    Abort;
  inherited;
...
procedure TFrmDLII041.DBGridEh1DblClick(Sender: TObject);
var
  s, sql: string;
begin
  inherited;
  ...
  begin
    ...
    begin
      ...
      sql := format(sql, [quotedstr(s), quotedstr(g_uinfo^.BU), quotedstr(cds.fieldbyname('dno').AsString), cds.fieldbyname
        ('ditem').asinteger]);
      cds.Edit;
...
procedure TFrmDLII041.DBGridEh1GetCellParams(Sender: TObject; Column: TColumnEh; AFont: TFont; var Background: TColor;
...
procedure TFrmDLII041.Timer1Timer(Sender: TObject);
var
  tmpSQL: string;
  Data: OleVariant;
begin
...
procedure TFrmDLII041.Timer2Timer(Sender: TObject);
var
  tmpSQL: string;
  Data: OleVariant;
begin
...
procedure TFrmDLII041.ToolBarDblClick(Sender: TObject);
begin
  inherited;
  Application.MessageBox('請更新VC測試值', '提示', MB_OK + MB_ICONINFORMATION);
end;
