...
uses
  ...
  Variants, Math, unGlobal, unCommon, unDLII020_upd, unACC58Service;
...
type
  ...
  public
    ...
    function SplitQtyAll(IsCCL:Boolean):Boolean;
...
//IsCCL:True是CCL False是PP
function TDLII020_btnopt.SplitQtyAll(IsCCL:Boolean):Boolean;
var
  tmpIndate:TDateTime;
...
begin
  ...
  begin
    ...
    begin
      ...
      tmpSQL:=Soap.ACC58_del(g_UInfo^.BU, tmpSaleno);
      if Pos('成功', tmpSQL)>0 then
         ShowMsg('出貨單號刪除完畢'+#13#10+tmpSQL,64)
      else
         ShowMsg('出貨單號刪除完畢'+#13#10+tmpSQL,48);
      Result:=True;
      Exit;
    end;
