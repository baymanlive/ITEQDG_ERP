  ...
  try
    ...
    begin
      ...
      tmpSQL:='SELECT azj04,azj03,azj04,azj03,azj04,azj04,''A'' AS A FROM azj_file'
             ...
             +' AND azk02<= to_date('+Quotedstr(StringReplace(FormatDateTime(g_cShortDate1,xDate),'-','/',[rfReplaceAll]))+','+Quotedstr(g_cShortDate1)+'))'
             +' AND ROWNUM=1'
             +' UNION ALL'
...
function SetData_Ogc(Dno, Ditem, Sno, AUnit, Custno: string;   
  OgbCDS,OgcCDS:TClientDataSet):Boolean;
              //ADD N2AF2,Y2AF2 2022/2/8
const stkplace='N2AF1,Y2AF1,N2AF2,Y2AF2,N0A00,Y0A00,N1A00,Y1A00,N3A18,D3A17,N0AM0,Y0AM0,N2A10,Y2A10,Y0AK0,Y0A01,Y0A02,Y0A03,Y0A04,Y0A05,Y0AM0,N0A01,N0A02,N0A03,N0A04,N0A05,N0AM0,';
const stkord1='222,227,22G,223,237,P2Z';
const stkord2='221,239,S11,SA1,S14,SB4,P1L,P18,225,220,239,229,P2Y';

const stkord1_pp='N2AF1,Y2AF1,N1A00,N3A18,N0AM0,N2A10';
const stkord1_ccl='N0A00,N3A18,N0AM0,Y0A01,Y0A02,Y0A03,Y0A04,Y0A05,Y0AM0,N0A01,N0A02,N0A03,N0A04,N0A05,N0AM0';

const stkord2_pp='Y1A00,D3A17,Y0AM0,Y2A10,Y0AK0';
const stkord2_ccl='Y0A00,D3A17,Y0AM0,Y0AK0,Y0A01,Y0A02,Y0A03,Y0A04,Y0A05,Y0AM0,N0A01,N0A02,N0A03,N0A04,N0A05,N0AM0';

var
  isDG:Boolean;
  tmpQty:Double;
  tmpSQL,tmpOraDB,tmpOgb04_1,tmpOgb31_3:string;
  Data:OleVariant;
  tmpCDS:TClientDataSet;
begin
  Result:=False;

  isDG:=Pos('dg', LowerCase(g_UInfo^.BU))>0;
  if isDG then
     tmpOraDB:='ORACLE'
  else
     tmpOraDB:='ORACLE1';
  ...
  try
    ...
    if isDG then
    begin
      tmpOgb04_1:=LeftStr(OgbCDS.FieldByName('ogb04').AsString,1);
      tmpOgb31_3:=LeftStr(OgbCDS.FieldByName('ogb31').AsString,3);

      tmpCDS.First;
      while not tmpCDS.Eof do
      begin
        if Pos(tmpCDS.FieldByName('stkplace').AsString,stkplace)=0 then
        begin
          ShowMsg('第'+Sno+'項,不是下列倉別,不可建單:'+#13#10+stkplace, 48);
          Exit;
        end;

        if Pos(tmpOgb31_3,stkord1)>0 then
        begin
          if Pos(tmpOgb04_1,'QMB')>0 then
          if Pos(tmpCDS.FieldByName('stkplace').AsString,stkord1_pp)=0 then
          begin
            ShowMsg('第'+Sno+'項,訂單別:'+stkord1+',倉別應為:'+#13#10+stkord1_pp, 48);
            Exit;
          end;

          if SameText(tmpOgb04_1,'T') then
          if Pos(tmpCDS.FieldByName('stkplace').AsString,stkord1_ccl)=0 then
          begin
            ShowMsg('第'+Sno+'項,訂單別:'+stkord1+',倉別應為:'+#13#10+stkord1_ccl, 48);
            Exit;
          end;
        end else
        if Pos(tmpOgb31_3,stkord2)>0 then
        begin
          if Pos(tmpOgb04_1,'RNP')>0 then
          if Pos(tmpCDS.FieldByName('stkplace').AsString,stkord2_pp)=0 then
          begin
            ShowMsg('第'+Sno+'項,訂單別:'+stkord2+',倉別應為:'+#13#10+stkord2_pp, 48);
            Exit;
          end;

          if SameText(tmpOgb04_1,'E') then
          if Pos(tmpCDS.FieldByName('stkplace').AsString,stkord2_ccl)=0 then
          begin
            ShowMsg('第'+Sno+'項,訂單別:'+stkord2+',倉別應為:'+#13#10+stkord2_ccl, 48);
            Exit;
          end;
        end;
    ...
    //ogb17多倉儲批出貨
    tmpCDS.First;
    if Pos(Custno, 'AC096/AC093/AC394/AC152/AH036')=0 then    //這些客戶建單要求多倉儲:名幸,廣州添利,惠亞,中山皆利
    begin
    ...
    begin
      ...
      begin
        ...
        begin
          FieldByName('ogc12').AsFloat:=RoundTo(tmpCDS.FieldByName('qty').AsFloat/FieldByName('ogc15_fac').AsFloat,-3);
          FieldByName('ogc16').AsFloat:=tmpCDS.FieldByName('qty').AsFloat;
        end else
        begin
        ...
        tmpQty:=tmpQty+tmpCDS.FieldByName('qty').AsFloat;
      end;
      tmpCDS.Next;
    end;

    if SameText(Trim(OgbCDS.FieldByName('ogb05').AsString),'RL') then   //合計數量、pp米換成卷
       tmpQty:=RoundTo(tmpQty/OgbCDS.FieldByName('ogb15_fac').AsFloat,-3);

    if Abs(RoundTo(OgbCDS.FieldByName('ogb12').AsFloat-tmpQty,-3))>0.00001 then
    begin
      ShowMsg('第'+Sno+'項,訂單['+OgbCDS.FieldByName('ogb31').AsString+'/'+
                                  OgbCDS.FieldByName('ogb32').AsString+
              ']檢貨數量與加總數量不一致,請核對!', 48);
...
//檢查資材與COC批號，數量是否一致
//Result=True檢查通過
function Check_ZC_COC_lot(Dno:string; Ditem:Integer):Boolean;
var
  tmpSQL,tmpLotErr:string;
  Data:OleVariant;
  tmpCDS:TClientDataSet;
begin
  //檢查資材批號
  ...
  tmpLotErr:='';
  Data:=null;
  tmpSQL:='exec dbo.proc_CheckZC_COC_lot '+Quotedstr(g_UInfo^.BU)+','+Quotedstr(Dno)+','+IntToStr(Ditem);
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if not tmpCDS.IsEmpty then
        begin
          tmpLotErr:=CheckLang('資材批號：')+#13#10;
          while not tmpCDS.Eof do
          begin
        ...
        if not tmpCDS.IsEmpty then
        begin
          tmpLotErr:=tmpLotErr+CheckLang('COC批號：')+#13#10;
          while not tmpCDS.Eof do
          begin
            tmpLotErr:=tmpLotErr+tmpCDS.Fields[0].AsString+#13#10;
            tmpCDS.Next;
      ...
      Result:=Length(tmpLotErr)=0;
      if not Result then
         ShowMsg('資材批號與COC批號(或數量)不一致'+#13#10+tmpLotErr,48);
    finally
      FreeAndNil(tmpCDS);
    end;
  end;
...
//廠內訂單不同,不可打在一起
const str_custno1='N005/ACA62/ACA12/AC142/AC129/AC108/AC082/AC151/AC769/AC625/AC103/AC136/ACA27/AC109/AC102/AC116/AC181/AC687/AC163/AC093/AC394/AC844/AC152/AH036/AC093/AC174';
//客戶訂單不同,不可打在一起+g_strCD
const str_custno2='AC052/ACA27/AC136/AC654/AC687/AC469/AC222/ACC91/ACD53';

var
  tmpOraDB:string;
  tmpogb17,isH,isHF:Boolean;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if SameText(FieldByName('custno').AsString,'AC051') then  //開平依利安達
        begin
          if Pos(Copy(FieldByName('Pno').AsString,2,1),'F6')>0 then       //有鹵
          begin
            if isHF then
            begin
              ShowMsg('有鹵無鹵物料不可打單在一起!', 48);
              Exit;
            end;

            isH:=True;
          end
          else if Pos(Copy(FieldByName('Pno').AsString,2,1),'HUP')>0 then //無鹵
          begin
            if isH then
            begin
              ShowMsg('有鹵無鹵物料不可打單在一起!', 48);
       ...
       { if Length(FieldByName('Custorderno').AsString)=0 then
        ...
        if SameText(g_UInfo^.BU,'ITEQDG') and SameText(FieldByName('custno').AsString,'AC135') then
        if not Check_ZC_COC_lot(FieldByName('dno').AsString,FieldByName('ditem').AsInteger) then
           Exit;
    ...
    try
      ...
      begin
        ...
        if SameText(g_UInfo^.BU,'ITEQDG') then
           FieldByName('ogagrup').AsString:='0D8120'    //資料所有部門
        else if SameText(g_UInfo^.BU,'ITEQGZ') then
           FieldByName('ogagrup').AsString:='0G8210'    
        else
           FieldByName('ogagrup').AsString:='XXXXXX';
      ...
      begin
        ...
        begin
          ShowMsg('訂單['+tmpCDS.FieldByName('Orderno').AsString+'/'+
      ...
      tmpStr := GetSaleNo_Id(tmpCDS.FieldByName('Orderno').AsString,0);
      if tmpStr='' then
      begin
