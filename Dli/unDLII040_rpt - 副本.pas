    ...
    begin
      ...
      begin
        if isLCA or SameText(SMRec.Custno, 'AC084') or (SameText(SMRec.Custno, 'AC111') and (Pos(SMRec.M2,
          '6/8/4/F/A/U/S/H') > 0)) or (SameText(SMRec.Custno, 'AC093') and SameText(SMRec.M2, 'E')) or ((Pos(SMRec.Custno,
          'AC394/AC152/AC082') > 0) and (Pos(SMRec.M2, '8/6/U/F/H') > 0)) then
        begin
          s1 := '200';
          s2 := '200';
        end
...
begin
  ...
  begin
    ...
    if Pos(SMRec.Custno, 'AC178,AC082,AC093,AC152') > 0 then
...
begin
  ...
  try
    ...
    FCDS.FieldByName('Fileno').AsString := 'Q1-' + FormatDateTime(g_cShortDateYYMMDD, tmpDate) + FSourceCDS.FieldByName('Coc_no').AsString;
    ...
    JxRemark(FSourceCDS.FieldByName('Remark').AsString, SMRec.Custno);
    ...
    begin
      ...
      if (ds3 <> nil) and (pos(SMRec.Custno, 'AC434/AC365/ACD39/AC388') > 0) then
      begin
        flag := false;
        ds3.first;
...
//    end;
    //廣州添利同訂單同料號不可以出現不同銅箔
    if SameText(SMRec.Custno, 'AC093') and (Length(FSourceCDS.FieldByName('CustOrderno').AsString) > 0) then
    begin
      Data := null;
...
//    else
    ...
    else if not ((SameText(SMRec.Custno, 'AC093') and (SMRec.M3_6 <= 20)) or   //非ac093 20mil以下,非AC109 15mil以下
      (SameText(SMRec.Custno, 'AC109') and (SMRec.M3_6 <= 15))) and ((SMRec.M3_6 > 12) or (Pos(SMRec.M7, '34567') > 0)
      or (Pos(SMRec.M8, '34567') > 0)) then
    begin
      FCDS.FieldByName('H1').AsString := '---';
      FCDS.FieldByName('H11').AsString := '---';
      FCDS.FieldByName('H12').AsString := 'NA';
    ...
    begin
      ...
      tmpSQL := 'Select *,case when charindex(' + Quotedstr(SMRec.MLast) + ',lastcode)>0 then ' + Quotedstr(SMRec.MLast)
        + ' else isnull(lastcode,'''') end lc ,case when charindex(' + Quotedstr(SMRec.MLast_1) + ',l1)>0 then ' + Quotedstr(SMRec.MLast_1)
        + ' else isnull(l1,'''') end l1 '+
        ' From Dli050 Where Bu=' + Quotedstr(g_UInfo^.BU) + tmpSQL +
...
//    else
   ...
   // end;
    ...
    //惠亞項目
    //華通、聯茂新埔TD
    if SameText(SMRec.Custno, 'AC109') or SameText(SMRec.Custno, 'AC145') or SameText(SMRec.Custno, 'N023') or (pos(SMRec.Custno,
      'AC950/AC075/AC310/AC311/AC405') > 0) then
    begin
      Data := null;
    ...
    //氯溴
    //STS
    if (SameText(SMRec.Custno, 'AC093') and (Pos(SMRec.M2, '4/8') > 0)) or (isHY and SameText(SMRec.M2, '6')) then
      FCDS.FieldByName('STS_visible').AsString := '1'
    else
      FCDS.FieldByName('STS_visible').AsString := '0';
    //STS
    //CustPno_visible
    tmpSQL := 'AC121/AC820/ACA97/AC109';
    if (Pos(SMRec.Custno, tmpSQL) > 0) and is968 then
      FCDS.FieldByName('CustPno_visible').AsString := '1'
    else
    begin
      tmpSQL :=
        'AC093/ACD67/AC148/AC082/AC135/AC109/AC405/AC310/AC311/AC075/AC347/AC734/AC950/AC121/AC076/AC360/N006/AC133/ACA28/ACE06/AC625/ACB00/ACD57/AC820/ACA97';
...
//    end
//    else
    if (Pos(SMRec.Custno, 'AC117/ACC19/AC136/ACA27/AC117/ACC19/AC136/ACA27') > 0) then
    begin
      FCDS.FieldByName('pp_visible').AsString := '1';
      FCDS.FieldByName('Copper_visible').AsString := '1';
      GetCCL_PP_Copper(SMRec.Custno, FSourceCDS.FieldByName('Dno').AsString, FSourceCDS.FieldByName('Ditem').AsString,
...
//    end;
  ...
  begin
    ...
    tmpSQL := 'Other-COCi'
  else if SameText(SMRec.Custno, 'AC145') then                       //全成信
    tmpSQL := 'Other-COCp'
