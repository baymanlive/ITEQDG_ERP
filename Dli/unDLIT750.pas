...
var
  FrmDLIT750: TFrmDLIT750;

implementation

uses unGlobal, unCommon, unDLIT7XX_units, unDLIT7XX_select;

const l_FormatStr='D75-YYMMDD999';

{$R *.dfm}

procedure TFrmDLIT750.CheckConf;
var
  tmpSQL:string;
  Data:OleVariant;
begin
...
//flag=0/1
procedure TFrmDLIT750.Conf(flag:string);
var
  v:Integer;
  tmpSQL:string;
  Data:OleVariant;
...
begin
  ...
  tmpSQL:='exec [dbo].[proc_DLIT750] '+Quotedstr(g_UInfo^.BU)+','+Quotedstr(CDS.FieldByName('cno').AsString)+','+flag;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        ShowMsg(tmpCDS.Fields[1].AsString+'棧板已出貨或已報廢,不可報廢!',48)
      else
...
procedure TFrmDLIT750.CdateChange(Sender: TField);
begin
...
procedure TFrmDLIT750.LotChange(Sender: TField);
var
  tmpSQL:string;
  tmpCDS:TClientDataSet;
  Data:OleVariant;
...
procedure TFrmDLIT750.RefreshDS2;
var
  tmpSQL:string;
begin
...
procedure TFrmDLIT750.FormCreate(Sender: TObject);
begin
  p_SysId:='Dli';
  p_MainTableName:='DLI750';
  p_DetailTableName:='DLI751';
...
procedure TFrmDLIT750.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrmDLIT750.btn_editClick(Sender: TObject);
begin
  CheckConf;
  inherited;
end;

procedure TFrmDLIT750.btn_deleteClick(Sender: TObject);
begin
  CheckConf;
  inherited;
end;

procedure TFrmDLIT750.btn_cancelClick(Sender: TObject);
begin
  inherited;
  imgConf.Visible:=CDS.FieldByName('Conf').AsBoolean;
end;

procedure TFrmDLIT750.btn_queryClick(Sender: TObject);
var
  tmpStr:string;
begin
  //inherited;
...
procedure TFrmDLIT750.btn_confClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmDLIT750.CDSNewRecord(DataSet: TDataSet);
begin
  inherited;
...
procedure TFrmDLIT750.CDSAfterOpen(DataSet: TDataSet);
begin
  inherited;
  DataSet.FieldByName('cdate').OnChange:=CdateChange;
end;

procedure TFrmDLIT750.CDS2AfterOpen(DataSet: TDataSet);
begin
  inherited;
  DataSet.FieldByName('lot').OnChange:=LotChange;
end;

procedure TFrmDLIT750.CDSBeforePost(DataSet: TDataSet);
var
  tmpSQL:string;
  isExist:Boolean;
begin
...
procedure TFrmDLIT750.CDS2BeforePost(DataSet: TDataSet);
begin
...
procedure TFrmDLIT750.CDSBeforeInsert(DataSet: TDataSet);
begin
  l_cno:='';
  inherited;
end;

procedure TFrmDLIT750.CDSBeforeDelete(DataSet: TDataSet);
begin
  inherited;
  l_cno:=DataSet.FieldByName('cno').AsString;
end;

procedure TFrmDLIT750.CDSBeforeEdit(DataSet: TDataSet);
begin
  inherited;
  l_cno:=DataSet.FieldByName('cno').AsString;
end;

procedure TFrmDLIT750.CDSAfterDelete(DataSet: TDataSet);
var
  tmpSQL:string;
begin
  inherited;
...
procedure TFrmDLIT750.CDSAfterPost(DataSet: TDataSet);
var
  tmpSQL:string;
begin
  inherited;
...
procedure TFrmDLIT750.CDSAfterScroll(DataSet: TDataSet);
begin
  inherited;
  imgConf.Visible:=DataSet.FieldByName('Conf').AsBoolean;
end;

procedure TFrmDLIT750.CDS2NewRecord(DataSet: TDataSet);
begin
  inherited;
  DataSet.FieldByName('cno').AsString:=CDS.FieldByName('cno').AsString;
end;

procedure TFrmDLIT750.CDS2BeforeInsert(DataSet: TDataSet);
begin
  CheckConf;
  inherited;
end;

procedure TFrmDLIT750.CDS2BeforeEdit(DataSet: TDataSet);
begin
  CheckConf;
  inherited;
end;

procedure TFrmDLIT750.CDS2BeforeDelete(DataSet: TDataSet);
begin
  CheckConf;
  inherited;
end;

procedure TFrmDLIT750.DBGridEh2EditButtonClick(Sender: TObject);
begin
  inherited;
  ...
  try
    FrmDLIT7XX_select.l_SQLFilter:=' and [state] not like ''出貨%'' and [state]<>''報廢''';
...
procedure TFrmDLIT750.Timer1Timer(Sender: TObject);
var
  tmpSQL:string;
  Data:OleVariant;
begin
