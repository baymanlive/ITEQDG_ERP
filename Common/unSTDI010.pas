...
{                Author: kaikai                         }
{                Create date:                           }
{                Description: 單筆作業:PageControl+Panel}
...
type
  TFrmSTDI010 = class(TFrmBaseEmpty)
    ToolBar: TToolBar;
...
var
  FrmSTDI010: TFrmSTDI010;

implementation

{$R *.dfm}

procedure TFrmSTDI010.SetToolBar;
var
  isEdit:Boolean;
begin
  ...
  begin
    Pnl1.Enabled:=True;
    Pnl2.Enabled:=True;
    if Self.FindComponent('grdEh_left')<>nil then
    if Self.FindComponent('grdEh_left') is TDBGridEh then
       TDBGridEh(Self.FindComponent('grdEh_left')).Enabled:=False;
  ...
  begin
    ...
    if Self.FindComponent('grdEh_left')<>nil then
    if Self.FindComponent('grdEh_left') is TDBGridEh then
       TDBGridEh(Self.FindComponent('grdEh_left')).Enabled:=True;
  end;
...
procedure TFrmSTDI010.SetSBars;
var
  dis_bo:Boolean;
...
procedure TFrmSTDI010.RefreshDS(strFilter:string);
begin
...
procedure TFrmSTDI010.FormCreate(Sender: TObject);
var
  i:Integer;
begin
  inherited;
...
procedure TFrmSTDI010.FormClose(Sender: TObject; var Action: TCloseAction);
var
  i:Integer;
begin
  inherited;
...
procedure TFrmSTDI010.btn_insertClick(Sender: TObject);
begin
  CDS.Append;
end;

procedure TFrmSTDI010.btn_editClick(Sender: TObject);
begin
  if CDS.IsEmpty then
     ShowMsg('無資料可更改!', 48)
  else
...
procedure TFrmSTDI010.btn_deleteClick(Sender: TObject);
begin
  if CDS.IsEmpty then
     ShowMsg('無資料可刪除!', 48)
...
procedure TFrmSTDI010.btn_copyClick(Sender: TObject);
var
  i:Integer;
  list: TStrings;
...
procedure TFrmSTDI010.btn_postClick(Sender: TObject);
begin
  CDS.Post;
end;

procedure TFrmSTDI010.btn_cancelClick(Sender: TObject);
begin
  if ShowMsg('確定放棄嗎?', 33)=IDOK then
     CDS.Cancel;
end;

procedure TFrmSTDI010.btn_printClick(Sender: TObject);
var
...
procedure TFrmSTDI010.btn_exportClick(Sender: TObject);
begin
  GetExportXls(CDS, p_TableName);
end;

procedure TFrmSTDI010.btn_queryClick(Sender: TObject);
var
  tmpStr:string;
begin
...
procedure TFrmSTDI010.btn_firstClick(Sender: TObject);
begin
  CDS.First;
end;

procedure TFrmSTDI010.btn_priorClick(Sender: TObject);
begin
  CDS.Prior;
end;

procedure TFrmSTDI010.btn_nextClick(Sender: TObject);
begin
  CDS.Next;
end;

procedure TFrmSTDI010.btn_lastClick(Sender: TObject);
begin
  CDS.Last;
end;

procedure TFrmSTDI010.btn_quitClick(Sender: TObject);
begin
...
procedure TFrmSTDI010.CDSAfterEdit(DataSet: TDataSet);
begin
...
procedure TFrmSTDI010.CDSAfterInsert(DataSet: TDataSet);
begin
...
procedure TFrmSTDI010.CDSAfterDelete(DataSet: TDataSet);
begin
...
procedure TFrmSTDI010.CDSAfterCancel(DataSet: TDataSet);
begin
...
procedure TFrmSTDI010.CDSAfterPost(DataSet: TDataSet);
begin
  if not CDSPost(CDS, p_TableName) then
  if CDS.ChangeCount>0 then
     CDS.CancelUpdates;
...
procedure TFrmSTDI010.CDSAfterScroll(DataSet: TDataSet);
begin
...
procedure TFrmSTDI010.CDSBeforeDelete(DataSet: TDataSet);
begin
  if not g_MInfo^.R_delete then
     Abort;
end;

procedure TFrmSTDI010.CDSBeforeEdit(DataSet: TDataSet);
begin
  if not g_MInfo^.R_edit then
     Abort;
end;

procedure TFrmSTDI010.CDSBeforeInsert(DataSet: TDataSet);
begin
  if not g_MInfo^.R_new then
     Abort;
end;

procedure TFrmSTDI010.CDSBeforePost(DataSet: TDataSet);
var
  ErrFName:string;
begin
...
procedure TFrmSTDI010.CDSNewRecord(DataSet: TDataSet);
var
  i:Integer;
begin
